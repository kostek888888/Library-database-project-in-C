...
#define CLEAR "clear"
    ...
    Borrow *borrow_base_pointer=NULL; /*!< wska«nik na baz© wypo¾yczeä */
...
/*!
 *   @brief funkcja sˆu¾y do zarejestrowania wypo¾yczenia ksi¥¾ki, sprawdza czy klient istnieje w bazie klient¢w oraz czy ksia¾ka jest dost©pna do wypo¾yczenia w bazie ksi¥¾ek je˜li tak wypo¾ycza i rejestruje to na li˜cie wypo¾yczeä
 *
...
void get_book(Library *base_pointer, Client *client_base_pointer){
    ...
    while(client_id_to_search<1){
        printf("\n Wprowadzono bˆ©dny numer id klienta, spr¢buj ponownie: ");
        scanf(" %d",&client_id_to_search);
    }
...
/*!
 *   @brief gˆ¢wna funkcja programu w kt¢rej u¾ytkownik wybiera operacje do wykonania, po wyj˜ciu z p©tli switch funkcja zapisuje wszystkie listy do plik¢w i kasuje je z pami©ci podr©cznej
 *
 *   @return 0
 */
int main(void)
