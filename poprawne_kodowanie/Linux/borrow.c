...
void new_borrow_fill(Borrow *(*new_borrow_node), int client_id, int book_id, int amount_to_borrow) {
    ...
    printf("\n Wpisz dzień wypożyczenia: ");
    scanf(" %d",&d_of_borrow);
    while(d_of_borrow<0||d_of_borrow>31){
        printf("\n Dzień miesiąca nie może przyjąć takiej wartości, spróbuj ponownie: ");
        scanf(" %d",&d_of_borrow);
    }
    ...
    printf("\n Wpisz miesiąc wypożyczenia: ");
    scanf(" %d",&m_of_borrow);
    while(m_of_borrow<0||m_of_borrow>12){
        printf("\n Miesiąc nie może przyjąć takiej wartości, spróbuj ponownie: ");
        scanf(" %d",&m_of_borrow);
    }
    ...
    printf("\n Wpisz rok wypożyczenia: ");
    scanf(" %d",&y_of_borrow);
    while(y_of_borrow<1950||y_of_borrow>2016){
        printf("\n Rok wypożyczenia nie może przyjąć takiej wartości, spróbuj ponownie: ");
        scanf(" %d",&y_of_borrow);
    }
...
void give_back_book(Borrow *borrow_base_pointer){
    ...
    printf("\n Wpisz numer id książki do oddania: ");
    scanf(" %d",&book_id);
    printf("\n Wpisz numer id klienta posiadającego tą książkę: ");
...
void cout_borrow_base(Borrow *borrow_base_pointer) {
    ...
    if (borrow_base_pointer!=NULL)
    {
        ...
        printf("ID książki: %d ",borrow_base_pointer->book_id);
        printf("data pożyczenia: %d ",borrow_base_pointer->d_of_borrow);
        printf("/%d",borrow_base_pointer->m_of_borrow);
        printf("/%d ",borrow_base_pointer->y_of_borrow);
        printf("pożyczona do: %d ",borrow_base_pointer->d_of_give_back);
    ...
    char buffer[6]="oddane";
        ...
        else
        {
            if(borrow_base_pointer!=NULL)
            {
                ...
                fprintf(file3," ID książki: %d",borrow_base_pointer->book_id);
                fprintf(file3," data pożyczenia: %d",borrow_base_pointer->d_of_borrow);
                fprintf(file3,"/%d",borrow_base_pointer->m_of_borrow);
                fprintf(file3,"/%d",borrow_base_pointer->y_of_borrow);
                fprintf(file3,"pożyczona do: %d",borrow_base_pointer->d_of_give_back);
